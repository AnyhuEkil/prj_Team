...
//	$(".whyBottomCommand").css({display:"none"});  //맵팀
	$(".whyBottomCommand:nth-child(1)").css({display:"block"});	//해당 클래스 이름을 변수로 가져올 방법이 있을꺼같음.. 나중에 추가
	yLocation = ".whyBottomCommand"; //해당 클래스 이름을 변수로 가져올 방법이 있을꺼같음.. 나중에 추가
	yBclCount = 2; //nth-child가 1번부터 이므로 1을씀(첫번째 것에 호버?를 해야하므로.)
	yLocationNth = yLocation+":nth-child("+yBclCount+")";
	$(yLocationNth).css({background:"purple",color:"black"});
	yakeyState=0;	//akey값 초기화
	yMl1st=1;
	yMl2nd=1;
	$(".whyTbWindow").css({display:"block"});	//해당 클래스 이름을 변수로 가져올 방법이 있을꺼같음.. 나중에 추가
}
/*
yMenuList[1]=[];
